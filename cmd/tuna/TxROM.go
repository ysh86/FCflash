...
func dumpTxromPRG(f io.Writer, s io.ReadWriter, prg int, buf []uint8) (err error) {
	// MMC3: PRG ROM R6:$C000-$DFFF swappable
	bankSelect := uint16(0b01000110)
	buf[0] = 0 // _reserverd
	buf[1] = uint8(REQ_CPU_WRITE_6502)
	binary.LittleEndian.PutUint16(buf[2:4], 0x9000)                // Value
	...
	for bank := 0; bank < banks; bank++ {
		buf[0] = 0 // _reserverd
		buf[1] = uint8(REQ_CPU_WRITE_6502)
		binary.LittleEndian.PutUint16(buf[2:4], 0x9001)                // Value
		binary.LittleEndian.PutUint16(buf[4:6], uint16(INDEX_IMPLIED)) // index
		...
		for i := 0; i < 0x2000; i += PACKET_SIZE {
			...
			binary.LittleEndian.PutUint16(buf[2:4], 0xc000|uint16(i))      // Value
			binary.LittleEndian.PutUint16(buf[4:6], uint16(INDEX_IMPLIED)) // index
